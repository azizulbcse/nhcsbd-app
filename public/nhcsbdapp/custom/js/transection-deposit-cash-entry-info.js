...
$(document).ready(function() {
	...
	manageTraxDepositInfoTable = $("#manageTraxDepositInfoTable").DataTable({
		'ajax': 'php_action/fetchDepositEntryInfo.php?id=1',
		'order': []		
	});
	...
	$("#submitDepositInfoForm").unbind('submit').bind('submit', function() {
		...
		if(YearlyAmount == "") {
			$("#YearlyAmount").after('<p class="text-danger">Fixed Amount To field is required</p>');
		...
		if(DepositDate || DepositTo || Remarks || YearlyAmount) {
			var form = $(this);
			// button loading
		...
		$.ajax({
			...
			success:function(response) {
				...
				$('#editDepositEntryInfoForm').unbind('submit').bind('submit', function() {
					...
					if(Remarks == "") {
						$("#editRemarks").after('<p class="text-danger">Edit Remarks field is required</p>');
