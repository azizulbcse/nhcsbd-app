...
$(document).ready(function() {
	...
	manageApplicantInfoTable = $("#manageApplicantInfoTable").DataTable({
		'ajax': 'php_action/fetchMemberProfile.php',
		'order': []		
	});
	...
	if(userid) {
		...
		$.ajax({
			...
			// alert(response.userpic);
				...
				$("#editAccountName").val(response.acc_name);
				$("#editMobileBankName").val(response.mobilebanktype);
				...
				$("#editMemberForm").unbind('submit').bind('submit', function() {
					...
					if(ApplicantNameBangla == "") {
						$("#editApplicantNameBangla").after('<p class="text-danger">Applicant Name Bangla field is required</p>');
                    ...
                    if(ApplicantNameEnglish == "") {
						$("#editApplicantNameEnglish").after('<p class="text-danger">Applicant Name English field is required</p>');
					...
					if(ApplicantNameBangla && ApplicantNameEnglish && FathersName && MothersName && Gender && MaritalStatus && 
					   ...
					   BloodGroup && NomineeName && NomineeRelation && NomineeMobile && NomineeAddress && EmergencyContact &&
					   BkashNogod && TransactionNo)  {
				...
				$("#updateMemberImageForm").unbind('submit').bind('submit', function() {					
					...
					if(memberImages == "") {
						$("#editUserImage").closest('.center-block').after('<p class="text-danger">Applicant Image field is required</p>');
					...
					if(memberImages) {
						...
						$.ajax({
				          ...
				          // reload the manage student table
									...
									$.ajax({
										url: 'php_action/fetchApplicantImageUrl.php?i='+mid,
										type: 'post',
										success:function(response) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#postedApplicantInfoBtn").unbind('click').bind('click', function() {
					...
					$.ajax({
						url: 'php_action/postApplicantInfo.php',
						type: 'post',
						data: {userid : userid},
						dataType: 'json',
						success:function(response) {
							...
							$.ajax({
								url: 'php_action/send-sms-approved.php',	
								type: 'post',
								data: {userid : userid},
								dataType: 'json',
								success:function(response) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#removeApplicantInfoBtn").unbind('click').bind('click', function() {
					...
					$.ajax({
						url: 'php_action/removeApplicantInfo.php',
						type: 'post',
						data: {userid : userid},
						dataType: 'json',
						success:function(response) {
							...
							$.ajax({
								url: 'php_action/send-sms-canceled.php',	
								type: 'post',
								data: {userid : userid},
								dataType: 'json',
								success:function(response) {
...
// print profile
function PrintMemberProfile(userid = null) {
	if(userid) {			
		$.ajax({
			url: 'php_action/printMemberProfile.php',
			type: 'post',
			data: {userid: userid},
			dataType: 'text',
			success:function(response) {
        ...
        mywindow.document.write('<html><head><title>Member Profile</title>');        
