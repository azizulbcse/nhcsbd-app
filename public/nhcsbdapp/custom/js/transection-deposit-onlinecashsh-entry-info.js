...
$(document).ready(function() {
	...
	manageTraxDepositInfoTable = $("#manageTraxDepositInfoTable").DataTable({
		'ajax': 'php_action/fetchDepositEntrySHInfo.php?id=2',
		'order': []		
	});
	...
	$("#submitDepositInfoForm").unbind('submit').bind('submit', function() {
		...
		if(Remarks == "") {
			$("#Remarks").after('<p class="text-danger">Remarks To field is required</p>');
		...
		if(YearlyAmount == "") {
			$("#YearlyAmount").after('<p class="text-danger">Yearly Amount To field is required</p>');
			$('#YearlyAmount').closest('.form-group').addClass('has-error');
		} else {
			// remov error text field
			$("#YearlyAmount").find('.text-danger').remove();
			// success out for form 
			$("#YearlyAmount").closest('.form-group').addClass('has-success');	  	
		}

		if(DepositDate || DepositForm || TrxNo || Remarks || YearlyAmount) {
			var form = $(this);
			// button loading
		...
		$.ajax({
			...
			success:function(response) {
				...
				$('#editDepositEntryInfoForm').unbind('submit').bind('submit', function() {
					...
					if(YearlyAmount == "") {
						$("#editRemarks").after('<p class="text-danger">Edit Fixed Amount field is required</p>');
						$('#editRemarks').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editRemarks").find('.text-danger').remove();
