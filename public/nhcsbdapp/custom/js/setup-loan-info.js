...
$(document).ready(function() {
	...
	manageResignReasonsInfoTable = $("#manageResignReasonsInfoTable").DataTable({
		'ajax': 'php_action/fetchSetupLoanTypeInfo.php', 
		'order': []		
	});
...
function editBankInfo(BankId = null) {
	...
	{ 
		...
		$.ajax({
			url: 'php_action/fetchSelectedSetupLoanType.php',
			type: 'post',
			data: {BankId : BankId},
			dataType: 'json',
			success:function(response) {
				...
				$('#editbankName').val(response.loantype);
				// brand id 
		...
		$.ajax({
			...
			success:function(response) {
				...
				$("#removeBankInfoBtn").unbind('click').bind('click', function() {
					...
					$.ajax({
						url: 'php_action/removeSetupLoanTypeInfo.php',
						type: 'post',
						data: {BankId : BankId},
						dataType: 'json',
						success:function(response) {
