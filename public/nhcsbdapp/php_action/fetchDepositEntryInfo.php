...
WHERE tdi.memberid = {$user_id} AND tdi.payment_type='Cash' AND tdi.memberid=tai.mid AND tdi.status!=0 ORDER BY tdi.trxdid DESC";
}
if($_GET['id']==2) 
{ 
...
$sql = "SELECT tdi.trxdid,DATE_FORMAT(tdi.depositdate, '%d/%m/%Y') as depositdate,tai.userpic,tdi.payment_type,tdi.deposit_from,
tdi.deposit_to,tdi.deposit_amount,tdi.fixed_amount,tdi.trxno,tdi.remarks,tdi.status FROM tbltrxdepositinfo tdi, tblapplicantinfo tai 
WHERE tdi.memberid = {$user_id} AND tdi.payment_type in ('Bkash','Nogod','Rocket','Upay') AND tdi.memberid=tai.mid AND tdi.status!=0 ORDER BY tdi.trxdid DESC";
}
